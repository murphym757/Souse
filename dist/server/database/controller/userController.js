...
exports.upload_user_image_mainpage = function (req, res, next) {
  var userName = req.params.username;
  var postTimestamp = req.params.timestamp;
  User.findOne({
    username: "" + userName + ""
  }).select("_id username").exec(function (err, user) {
    var userData = {
      _id: user._id,
      username: user.username
    };
    ...
    var userImageUpload = multer({
      ...
      storage: multerS3({
        ...
        key: function key(req, file, cb) {
          ...
          var mimeTypeTest = filetypes.test(file.mimetype);
          var newFileName = Date.now().toString();
          var fullPath = 'users/' + "" + userData._id + "" + '/posts/' + postTimestamp + '/' + userData._id + extname;
